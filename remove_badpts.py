...
def getgudpts_wspec(all_lat,all_lon,all_spec,boundary_path):
    # removes the points over the ocean and returns the array of species
    # all_lat and all_lon are numpy arrays of all the points, boundary_path is the path to the shapefile containing the california boundary
    ...
    # regular pandas dataframe for the points (with column for whether they are in the boundaries or not)
    d = np.vstack([all_lon,all_lat,all_spec,np.zeros(len(all_lon))]).T
    test_pts = pd.DataFrame(d,columns=['longitude','latitude','species','in bounds'])
    ...
    for idx,row in test_pts.iterrows():
        if row['in bounds']>0:
            ...
            temp_new_lat.append(temp1)
            temp_new_lon.append(temp2)
            temp_new_spec.append(row['species'])

    new_lat = np.array(temp_new_lat)
    new_lon = np.array(temp_new_lon)
    new_spec = np.array(temp_new_spec)
    
    print(new_lat)
    print(new_lon)

    return new_lat,new_lon,new_spec
    # plt.show()
